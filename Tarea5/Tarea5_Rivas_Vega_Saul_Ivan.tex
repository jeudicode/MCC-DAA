...
\section{Ejercicio 4.}
Diseña un algoritmo tipo $greedy$ para el siguiente problema de optimización. La entrada está compuesta de un conjunto con $n$ pares de enteros positivos, $\{(w_1, v_1), ..., (w_n, v_n)\}$, y un entero positivo $W$; cada $(w_i, v_i)$ representa una clase de objetos con peso $w_i$ y $valor_i$, y $W$ representa una caja que puede cargar esa cantidad de peso. El objetivo es seleccionar objetos a meter en la caja de forma tal que se maximice la suma del valor de los objetos seleccionados, permitiendo meter fracciones de objetos así como repeticiones; por ejemplo, tu algoritmo podría decidir meter dos veces el mismo objeto, una de ellas una unidad entera y la segunda solo una fracción.\\
\begin{algorithm}[H]\small
	\KwData{Un conjunto de $n$ pares $O$ de la forma $(w_i, v_i)$ que representa el conjunto de objetos.}
	\KwData{Un entero positivo $W$ que representa la capacidad de peso de la caja.}
	\KwResult{Una tripleta $(valor\_maximo, id, cantidad)$ que representa el valor maximo alcanzado, el identificador del objeto tomado, y la cantidad (que puede ser fraccional) que se tomó de dicho objeto.}
	\SetAlgoLined
	\tcc{Inicializar una variable para guardar la mayor relación valor/peso.}
	$mayor\_relacion\_vp:=-\infty$\;
	\tcc{Inicializar una variable para guardar el valor máximo obtenido.}
	$valor\_maximo:=0$\;
	\tcc{Inicializar una variable para guardar el identificador del objeto a tomar.}
	$id\_objeto:=0$\;
	\tcc{Inicializar una variable para guardar el peso del objeto tomado.}
	$peso\_objeto:=0$\;
	\tcc{Inicializar una variable para guardar la cantidad tomada del objeto.}
	$cantidad\_objeto:=0$\;
	\tcc{Recorremos cada posición de las parejas de la entrada.}
	\For{$i$ desde $1$ hasta $|O|$}{
			\tcc{Obtenemos los valores del objeto en la posición $i$.}
			$(w_i, v_i):= O[i]$\;
    		\tcc{Si la relación valor/peso del objeto actual es mayor a la que teníamos almacenada, la actualizamos.}
			\If{$v_i / w_i > mayor\_relacion\_vp$}{
				$mayor\_relacion\_vp:=v_i / w_i$\;
				$id\_objeto:=i$\;
				$peso\_objeto:=w_i$\;
			}
	}
	\tcc{Asignamos el mayor valor que podemos obtener.}
	$valor\_maximo:=W \times mayor\_relacion\_vp$\;
	\tcc{Asignamos la cantidad tomada del objeto.}
	$cantidad\_objeto:=W / peso\_objeto$\;
	return $(valor\_maximo, id\_objeto, cantidad\_objeto)$\;
