 ...
 \begin{algorithm}[H]
 	...
 	\KwData{Índice $i$ del elemento a eliminar}
		...
		Eliminamos el elemento en $n-1$
 		\eIf{El elemento en $i$ ahora es menor a su padre.}{
 				SUBIR($i$)\;
 		}{
 		\If{alguno de sus hijos es menor}{
 				BAJAR($i$)\;
 		}
 	}
...
\begin{algorithm}[H]
	\SetAlgoLined
	\KwData{Índice $i$ del elemento a subir}
	Reemplazamos el valor de $i$ con el de su padre.\;
	\If{El elemento en la nueva posición sigue siendo menor a su nuevo padre.}{
		SUBIR($i$)\;
	}
...
\begin{algorithm}[H]
	\SetAlgoLined
	\KwData{Índice $i$ del elemento a bajar}
	Reemplazamos el valor de $i$ con el del menor de sus hijos.\;
	\If{El elemento en la nueva posición sigue siendo mayor que alguno de sus nuevos hijos.}{
		BAJAR($i$)\;
	}
